...
test('can get a deeply embedded node', () => {
  ...
  const value = getASTNode(node, 'children[0].children[0].children[0]')
...
test('can get a deeply embedded token', () => {
  ...
  const value = getToken(node, 'children[0].children[0].children[0].children[0]')
...
test('can access different tokens', () => {
  ...
  // expr -> path -> token[0]
  const token = getToken(node, 'children[0].children[0].children[0].children[0]')
